...
export function NoteList({ notes, selectedNoteId, onSelectNote }: NoteListProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto" role="navigation" aria-label="Notes list">
      <ul>
        {notes.map((note) => (
          <li key={note.id}>
            <button
            ...
            >
              <div className="flex items-start gap-3">
                <FileText className="h-4 w-4 mt-1 text-muted-foreground shrink-0" aria-hidden="true" />
